...
/**
 * Provides a friendly event driven API to cryptorescued in Node.js. Manages starting and
 * stopping cryptorescued as a child process for application support, as well as connecting
 * to multiple cryptorescued processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function CryptoRescue(options) {
  if (!(this instanceof CryptoRescue)) {
    return new CryptoRescue(options);
  }
...
util.inherits(CryptoRescue, Service);

CryptoRescue.dependencies = [];

CryptoRescue.DEFAULT_MAX_TXIDS = 1000;
CryptoRescue.DEFAULT_MAX_HISTORY = 50;
CryptoRescue.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
CryptoRescue.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
CryptoRescue.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
CryptoRescue.DEFAULT_SPAWN_RESTART_TIME = 5000;
CryptoRescue.DEFAULT_SPAWN_STOP_TIME = 10000;
CryptoRescue.DEFAULT_TRY_ALL_INTERVAL = 1000;
CryptoRescue.DEFAULT_REINDEX_INTERVAL = 10000;
CryptoRescue.DEFAULT_START_RETRY_INTERVAL = 5000;
CryptoRescue.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
CryptoRescue.DEFAULT_TRANSACTION_CONCURRENCY = 5;
CryptoRescue.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'cryptorescuecore'
};

CryptoRescue.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || CryptoRescue.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || CryptoRescue.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || CryptoRescue.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || CryptoRescue.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || CryptoRescue.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Cryptorescue.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || CryptoRescue.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || CryptoRescue.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || CryptoRescue.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || CryptoRescue.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

CryptoRescue.prototype._initCaches = function() {
...
CryptoRescue.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
CryptoRescue.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
CryptoRescue.prototype.getPublishEvents = function() {
  return [
    {
    ...
    {
      name: 'cryptorescued/addresstxid',
      scope: this,
...
CryptoRescue.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'cryptorescued/' + name, 'total:', this.subscriptions[name].length);
};

CryptoRescue.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'cryptorescued/' + name, 'total:', this.subscriptions[name].length);
};

CryptoRescue.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  for(var i = 0; i < addresses.length; i++) {
    if (cryptorescuecore.Address.isValid(addresses[i], this.node.network)) {
      addAddress(addresses[i]);
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'cryptorescued/addresstxid', 'total:', _.size(this.subscriptions.address));
};

CryptoRescue.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'cryptorescued/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
CryptoRescue.prototype.unsubscribeAddressAll = function(emitter) {
...
CryptoRescue.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = CryptoRescue.DEFAULT_CONFIG_SETTINGS;
...
CryptoRescue.prototype._parseCryptoRescueConf = function(configPath) {
  var options = {};
...
CryptoRescue.prototype._expandRelativeDatadir = function() {
...
CryptoRescue.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in cryptorescued config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in cryptorescued config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in cryptorescued config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './cryptorescue.conf');

  log.info('Using cryptorescue config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseCryptoRescueConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseCryptoRescueConf(networkConfigPath));
  }
...
CryptoRescue.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of cryptorescuecore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of cryptorescuecore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of cryptorescuecore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to cryptorescued from cryptorescuecore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from cryptorescued. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from cryptorescued. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in cryptorescue.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that cryptorescued is undergoing a reindex. ' +
             ...
             'of cryptorescuecore-node services will start.');
    node._reindex = true;
  }
};

CryptoRescue.prototype._resetCaches = function() {
...
CryptoRescue.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
CryptoRescue.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

CryptoRescue.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('CryptoRescue Daemon Ready');
          callback();
        });
      });
...
CryptoRescue.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8766
  };
  if (this.node.network === cryptorescuecore.Networks.testnet) {
    networkOptions.rpcport = 18766;
  }
  return networkOptions;
};

CryptoRescue.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === cryptorescuecore.Networks.testnet) {
    networkPath = 'testnet3/cryptorescue.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/cryptorescue.conf';
    }
  }
  return networkPath;
};

CryptoRescue.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === cryptorescuecore.Networks.testnet) {
...
CryptoRescue.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('cryptorescued/hashblock', message.toString('hex'));
    }
  }

};

CryptoRescue.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
CryptoRescue.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('CryptoRescue Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
CryptoRescue.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
CryptoRescue.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('cryptorescued/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
CryptoRescue.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = cryptorescuecore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('cryptorescued/rawtransaction', message.toString('hex'));
    }
...
CryptoRescue.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || CryptoRescue.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

CryptoRescue.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
CryptoRescue.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
CryptoRescue.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('CryptoRescue Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || CryptoRescue.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

CryptoRescue.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
CryptoRescue.prototype._stopSpawnedCryptoRescue = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/cryptorescued.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned CryptoRescue process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean CryptoRescue process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
CryptoRescue.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedCryptoRescue(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting CryptoRescue process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('CryptoRescue process unexpectedly exited with code:', code);
        log.warn('Restarting CryptoRescue child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('CryptoRescue process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new CryptoRescueRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn cryptorescued.'));
      }
...
CryptoRescue.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to cryptorescued.'));
    }
 ...
 */
CryptoRescue.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('CryptoRescue configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.syncPercentage = function(callback) {
  var self = this;
...
CryptoRescue.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, cryptorescuecore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
CryptoRescue.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = cryptorescuecore.Script.fromAddress(delta.address);
    return {
...
CryptoRescue.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
CryptoRescue.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
CryptoRescue.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in cryptorescuecore is most recent to less recent
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
CryptoRescue.prototype._getConfirmationsDetail = function(transaction) {
...
CryptoRescue.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
CryptoRescue.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
CryptoRescue.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A CryptoRescue transaction id
 * @param {Function} callback
 */
CryptoRescue.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
CryptoRescue.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof cryptorescuecore.Address) {
...
CryptoRescue.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
CryptoRescue.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a cryptorescuecore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
CryptoRescue.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = cryptorescuecore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
CryptoRescue.prototype.estimateSmartFee = function(blocks, conservative, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a cryptorescuecore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
CryptoRescue.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.getInfo = function(callback) {
  var self = this;
...
CryptoRescue.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
CryptoRescue.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('cryptorescued spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('cryptorescued process did not exit'));
      }
