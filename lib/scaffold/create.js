...
var BASE_PACKAGE = {
  description: 'A full CryptoRescue node build with CryptoRescueCore',
  repository: 'https://github.com/cryptorescue-project/cryptorescuecore',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
...
/**
 * Will create a directory and cryptorescue.conf file for CryptoRescue.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createCryptoRescueDirectory(datadir, done) {
...
/**
 * Will create a base CryptoRescueCore Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The cryptorescue database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/cryptorescuecore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a CryptoRescueCore Node directory, configuration file,
 * cryptorescue configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the cryptorescuecore node configuration directory
 * @param {String} options.datadir - The path to the cryptorescue datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the cryptorescuecore-node directory and configuration
    ...
    function(next) {
      // Setup the cryptorescue directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createCryptoRescueDirectory(absDataDir, next);
      } else {
        next();
      }
    },
