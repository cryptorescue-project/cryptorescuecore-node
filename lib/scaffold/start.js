...
var CryptoRescueCoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for cryptorescued is necessary for this upgrade with the "reindex=1" cryptorescue.conf option. \n' +
                 'There are changes necessary in both cryptorescue.conf and cryptorescuecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/cryptorescue-project/cryptorescuecore-node/blob/master/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed cryptorescuecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './cryptorescuecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in cryptorescuecore-node services directory
    ...
    if (servicePackage.cryptorescuecoreNode) {
      serviceModule = service.name + '/' + servicePackage.cryptorescuecoreNode;
    }
 ...
 * @param {Node} node - The CryptoRRescueCore Node instance
 */
