...
/**
 * Will remove a service from cryptorescuecore-node.json
...
/**
 * Will remove the Node.js service and from the cryptorescuecore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The cryptorescuecore-node configuration directory
...
function remove(options, done) {
  ...
  var cryptorescuecoreConfigPath = path.resolve(configPath, 'cryptorescuecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(cryptorescuecoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a cryptorescuecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to cryptorescuecore-node.json
        removeConfig(cryptorescuecoreConfigPath, service, next);
      });
    }, done
  );
}
