...
/**
 * Will return the path and default cryptorescuecore-node configuration. It will search for the
 * configuration file in the "~/.cryptorescuecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.cryptorescuecore');
  var defaultConfigFile = path.resolve(defaultPath, './cryptorescuecore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['cryptorescued', 'web'];
