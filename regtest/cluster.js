...
describe('CryptoRescue Cluster', function() {
  var node;
  var daemons = [];
  var execPath = path.resolve(__dirname, '../bin/cryptorescued');
  var nodesConf = [
    {
      datadir: path.resolve(__dirname, './data/node1'),
      conf: path.resolve(__dirname, './data/node1/cryptorescue.conf'),
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node2/cryptorescue.conf'),
      rpcuser: 'cryptorescue',
      rpcpassword: 'local321',
      rpcport: 30522,
    ...
    },
    {
      ...
      conf: path.resolve(__dirname, './data/node3/cryptorescue.conf'),
      rpcuser: 'cryptorescue',
      rpcpassword: 'local321',
      rpcport: 30523,
  ...
  before(function(done) {
    log.info('Starting 3 cryptorescued daemons');
    this.timeout(60000);
    async.each(nodesConf, function(nodeConf, next) {
      ...
      rimraf(path.resolve(nodeConf.datadir, './regtest'), function(err) {
        ...
        var client = new CryptoRescueRPC({
          protocol: 'http',
          host: '127.0.0.1',
  ...
  it('step 1: will connect to three cryptorescued daemons', function(done) {
    this.timeout(20000);
    var configuration = {
      network: 'regtest',
      services: [
    ...
    var regtest = cryptorescuecore.Networks.get('regtest');
    should.exist(regtest);

    node = new CryptoRescueCoreNode(configuration);
  ...
  it('step 2: receive block events', function(done) {
    this.timeout(10000);
    node.services.cryptorescued.once('tip', function(height) {
